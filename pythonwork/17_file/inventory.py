...
def findproductName(sub):
    ...
    if subSelect.isdigit():
        ...
        print("품목번호는 숫자만 가능합니다");
        return -100;

dirPath = "C:\\20240416\\pythonwork\\product\\";
productName = os.listdir(dirPath); #폴더 안의 파일명을 리스트로 만듦
today = datetime.today().strftime("%Y-%m-%d"); #스트링 형식의 시간
...
while True:
    ...
    if select == "1":
        ...
        for i in productName:
            ...
            open(dirPath + proName, 'w+', encoding="utf-8"); # w+ 는 읽기or쓰기모드 파일이 없으면 생성함
        ...
        if subSelect!= -100:
            if 0 <= subSelect <= len(productName)-1:
                print(f"{productName[subSelect]}품목을 삭제합니다.")
                os.remove(dirPath+productName[subSelect]); #폴더에서 파일 자체를 삭제
                productName.pop(subSelect); #파일명 리스트에서 파일명 삭제
            else:
                print("없는 품목 번호 입니다.");
    elif select == "0":
        print("프로그램을 종료 합니다.");
