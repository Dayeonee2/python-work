# 로그인 한 사람만 접근이 가능
# 로그인 한 자신의 폴더에 접근 가능해서 일기 쓰기와 보기

import diary

loginPath="C:\\20240416\\pythonwork\\final\\member";
...
while True:
    ...
    if select =="1":
        ...
        try:
            while True:
                print("회원가입 취소시 Ctrl + Z 를 입력하세요.")
                with open (loginPath,"r",encoding = "utf-8") as f:
                    member = f.readlines();
                    for m in member:
                        m = m.split(",");
                        if id == m[0] :
                            print("중복된 아이디 입니다. 다시 입력하세요.")
                            break;
                    else:
                        break;
            pwd = input("비밀번호 입력 : ");
